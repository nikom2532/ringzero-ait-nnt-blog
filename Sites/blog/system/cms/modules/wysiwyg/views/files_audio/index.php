...
<div id="files_browser">
	...
	<div id="files_right_pane">
		...
		<?php if ($current_folder): ?>
			<h3>โฟลเดอร์ : <?php echo $current_folder->name ?> <span style="color:red">[ไฟล์ออดิโอ]</span></h3>  
			<!-- subfolders -->
			...
			?>
				<?php
					echo '<br><h1 style="color:red;">'.$title_prd.'</h1>';
				?>
			<table class="table-list" border="0" cellspacing="0">
				...
				<!--
					...
					<!-- insert form prd -->
						<?php 
							$ii =1;
							foreach ($get_video as $news_item): 
						...
						<tr class="<?php echo alternator('', 'alt') ?>">
							<td class="image">
								<button onclick="javascript:insertAudio('<?php echo $news_item->NT10_VDOID."', '".htmlentities(addslashes($news_item->NT10_VDOName))."', '".$news_item->url."', '".$news_item->url ?>');">
									Insert
								</button>
							</td>
							<td class="name-description">
								<p><?php echo $news_item->NT10_VDOName ?><p>
								<p><?php echo $news_item->url ?></p>
							</td>
							<td class="filename">
								<!--<p><?php echo $file->filename ?></p>-->
								<p><?php echo format_date($file->date_added) ?></p>
						...
						<?php
							...
							//echo '<video src="'.$news_item->url.'" controls>
